...
test("the graded R:R is computed from geometry, not from plan.rr", () => {
  const setup = deriveSetup(null, engineRow);
  const p = buildSignalPayload({
    raw: engineRaw, sig: gated, row: engineRow, setup,
    provenance: buildProvenance(engineRaw, gated, {}),
  ...
  // entry 641.20, stop 637.90 -> risk 3.30; t1 646.15 -> reward 4.95 = 1.5R.
  // plan.rr says 3.0, which is the t2 multiple and is never graded.
  assert.equal(Number(p.plan.rr_graded).toFixed(2), "1.50", "graded ratio is t1-based");
  assert.equal(p.plan.rr_declared, "3", "declared value preserved for audit");
  assert.equal(p.plan.rr_declared_is_graded, false);
  assert.equal(p.plan.rr_declared_refers_to, "t2");
  assert.equal(p.plan.rr, undefined, "the ambiguous single scalar must not be emitted");
});

test("degenerate geometry yields null rather than a wrong number", () => {
  const flat = { ...engineRow, plan: { entry: 100, stop: 100, t1: 105, t2: 110, rr: 3.0 } };
  const p = buildSignalPayload({
    raw: engineRaw, sig: gated, row: flat, setup: deriveSetup(null, flat),
    provenance: buildProvenance(engineRaw, gated, {}),
  });
  assert.equal(p.plan.rr_graded, null, "zero risk leg must not divide");

  const partial = { ...engineRow, plan: { entry: 100, stop: null, t1: 105 } };
  const q = buildSignalPayload({
    raw: engineRaw, sig: gated, row: partial, setup: deriveSetup(null, partial),
    provenance: buildProvenance(engineRaw, gated, {}),
  });
  assert.equal(q.plan.rr_graded, null, "incomplete geometry must not guess");
});
...
// ── the bug that made every real signal fail ───────────────────────────────

test("REGRESSION: raw engine floats never reach canonicalization", async () => {
  const { buildEnvelope } = await import("./labContract/envelope.js");

  // A realistic engine object: structure/confirmation/risk are full of swing
  // prices and levels, none of them integers.
  const floaty = {
    ...engineRaw,
    structure: {
      lastSwingLow: 637.9123, lastSwingHigh: 651.44,
      fvg: [{ lo: 640.25, hi: 641.875 }, { lo: 0.1 + 0.2, hi: 1e-7 }],
      events: [{ at: "2026-07-27T17:00:00Z", level: 644.3333333 }],
    },
    confirmation: { confirmed: true, score: 0.8271, threshold: 0.75 },
    risk: { approved: true, atr: 3.4219, sizeFraction: 0.0125 },
    price: 641.2049,
  };

  const setup = deriveSetup(null, engineRow);
  const payload = buildSignalPayload({
    raw: floaty, sig: gated, row: engineRow, setup,
    provenance: buildProvenance(floaty, gated, { minConviction: 65 }),
  });

  // Before the fix this threw CanonicalizationError inside buildEnvelope, was
  // swallowed by the synchronous catch, and reported as failed-with-no-error.
  const env = buildEnvelope({
    eventType: "signal.emitted", payload,
  ...
  const st = env.payload.provenance.structure;
  assert.equal(st.lastSwingLow, "637.9123", "floats become decimal strings");
  assert.equal(st.fvg[0].hi, "641.875");
  assert.equal(typeof env.payload.provenance.confirmation.score, "string");
  assert.equal(env.payload.provenance.candle_count, 480, "integers stay integers");
});

test("REGRESSION: a construction failure records its error", async () => {
  // A payload that cannot be canonicalized at all, to prove LAST_ERROR is set
  // rather than the failure being silent.
  const { labLastError } = await import("./labEmitter.js");
  const bad = { ...engineRow, plan: { entry: { toString() { throw new Error("boom"); } } } };
  await emitSignal({ raw: engineRaw, sig: gated, row: bad, last: null });
  assert.notEqual(labLastError(), null, "a build failure must not be silent");
});
