...
const LAB_URL = process.env.KRONOS_LAB_URL || "";
const LAB_KEY = process.env.KRONOS_LAB_HMAC_KEY || "";
...
async function post(envelope) {
  ...
  try {
    ...
    if (!res.ok) console.warn(`[lab] ingest ${res.status} for ${envelope.event_type}`);
    return res.ok;
  } catch (e) {
    // AbortError included: a slow Lab must never slow signal generation.
    console.warn(`[lab] emit failed (${envelope.event_type}): ${e?.message ?? e}`);
    return false;
  } finally {
    clearTimeout(timer);
  }
