...
export function buildProvenance(raw, sig, opts = {}) {
  ...
  return {
    ...
    //
    // agentsFull, NOT agents. signalEngine:340 strips `data` in its own return,
    // so reading raw.agents here captured null for every agent of every signal
    // -- 620 rows recorded with the one field the Lab most wanted empty. The
    // comment above was right about the problem and the source was wrong.
    agent_data: Array.isArray(raw?.agentsFull ?? raw?.agents)
      ? (raw.agentsFull ?? raw.agents).map((a) => ({ agent: a.agent, data: a.data ?? null }))
      : null,

    // Market context as of this decision. Supplied by the caller, never looked
    // up here -- see lib/regime.js.
    regime: opts.regime ?? null,

    // Geometry rejections. A signal whose stop sat on the wrong side of entry
    // used to be written and then grade itself won within seconds; it is now
    // refused outright, and this records that it happened rather than leaving
    // a silent gap where a plan should be.
    plan_rejected: raw?.planRejected ?? null,

    // Dealer gamma and the catalyst backdrop as of this decision. Recorded so
    // the Lab can test whether either preceded the moves that ran -- neither
    // influenced the signal that produced them.
    gex: raw?.gex ?? null,
...
/**
 * Recursively convert every non-integer number to a decimal string.
 *
 * canonicalize() THROWS on raw floats -- deliberately, because Python and
 * JavaScript render some IEEE-754 doubles differently and a float reaching the
 * hasher makes the two sides disagree forever. That rule was enforced on
 * `plan` but not on the raw engine objects (structure, confirmation, risk),
 * which are full of swing prices, FVG levels and ATR values. Every real signal
 * therefore threw inside buildEnvelope before any request was made.
 *
 * Integers are left alone: they canonicalize identically in both languages.
 */
function floatsToStrings(v, depth = 0) {
  if (depth > 12) return null;                       // engine objects can nest; refuse to recurse forever
  if (v === null || v === undefined) return v ?? null;
  const t = typeof v;
  if (t === "number") {
    if (!Number.isFinite(v)) return null;            // NaN/Infinity have no wire representation
  ...
  if (Array.isArray(v)) return v.map((x) => floatsToStrings(x, depth + 1));
  if (t === "object") {
    const out = {};
    for (const [k, val] of Object.entries(v)) {
      const conv = floatsToStrings(val, depth + 1);
      if (conv !== undefined) out[k] = conv;
    }
    return out;
  }
  return null;
...
export function buildSignalPayload({ raw, sig, row, setup, provenance }) {
  ...
  return {
    ...
    provenance: floatsToStrings(provenance),
  };
}
...
export function emitSignal({ raw, sig, row, last, opts = {} }) {
  ...
  try {
    ...
    // Construction failures (canonicalization, payload shape) happen BEFORE
    // post() runs, so without this they leave LAST_ERROR null and the
    // telemetry reads "failed with no error" -- which is what sent us chasing
    // the network for an hour.
    LAST_ERROR = `build: ${e?.name ?? "Error"}: ${e?.message ?? e}`;
    console.warn(`[lab] emitSignal skipped: ${LAST_ERROR}`);
...
export function emitOutcome({ signalId, setupId, state, plan, resolvedAt, mode = "engine", actor = null, reason = null,
                              excursion = null, pathAmbiguous = null, supersededBy = null, priceAtResolution = null,
                              disposition = null, dispositionNote = null, minutesToDisposition = null }) {
  if (!labEnabled()) return Promise.resolve(false);
  try {
    const payload = {
      ...
      // What the trade did on the way, in R. Decimals as strings like every
      // other float here -- raw doubles render differently in Node and Python
      // and would break the content hash.
      mae_r: dec(excursion?.mae_r),
      mfe_r: dec(excursion?.mfe_r),
      bars_to_resolution: Number.isFinite(excursion?.bars_seen) ? excursion.bars_seen : null,
      price_at_resolution: dec(priceAtResolution),
      // Realised R by the geometry actually graded: t1 sits at 1.5R and the
      // stop at 1R. NOT plan.rr, which is hardcoded 3.0 and refers to t2 --
      // a value wrong by exactly 2x is worse than an absent one.
      r_realized: state === "won" ? "1.5" : state === "lost" ? "-1.0" : null,
      // The grader tests t1 before the stop against a single spot price. This
      // says the recorded excursions contradict the verdict it reached.
      path_ambiguous: pathAmbiguous,
      superseded_by: supersededBy,

      // A human's call, and only ever a human's. The engine leaves these null,
      // which is precisely what makes discretionary removals countable and
      // separable from the outcomes it graded itself.
      disposition,
      disposition_note: dispositionNote,
      minutes_to_disposition: Number.isFinite(minutesToDisposition) ? minutesToDisposition : null,

      // Peak excursion in PERCENT of entry. R says how a trade did against its
      // own risk; percent is the only unit in which a 10% move and a 100% move
      // are different things.
      peak_gain_pct: dec(excursion?.peak_gain_pct),
      peak_loss_pct: dec(excursion?.peak_loss_pct),
    ...
    LAST_ERROR = `build: ${e?.name ?? "Error"}: ${e?.message ?? e}`;
    console.warn(`[lab] emitOutcome skipped: ${LAST_ERROR}`);
    return Promise.resolve(false);
  }
}
