...
import { signalMatchesPrefs, pushBlockReason } from "./alertPrefs";
...
export async function sendSignalPush(sig) {
  ...
  await Promise.all((subs || []).map(async (s) => {
    // ONE gate function, shared with /api/push/diagnose — see pushBlockReason.
    if (pushBlockReason(sig, s)) return;
    try {
