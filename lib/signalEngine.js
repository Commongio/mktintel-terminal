...
import { geometryValid } from "./geometry";
import { symbolCatalyst, macroCatalyst, mergeCatalyst } from "./catalyst.js";
...
export const MODE_DEFAULT_INTERVAL = { futures: "15min", options: "1h", equity: "1w" }; // V14: equity default is the MONTHLY horizon (1w candles) — "1d" was dropped with the weekly horizon
...
// Finite numbers only. A NaN or a null slipping into the geometry check would
// make every comparison false, which reads as "invalid" and silently drops
// good signals -- so the guard is explicit rather than relying on coercion.
...
// V14: the take-profit horizon for a long-term (INVEST) signal, as a concrete
// calendar target the user can actually act on. The weekly horizon was removed —
// INVEST is Monthly or Yearly only:
//   "1w"  candles → MONTHLY horizon → take profit ~3 months out
//   "1mo" candles → YEARLY  horizon → take profit ~12 months out
// Returned as both a label and machine-readable parts so the UI can render it
// without re-deriving dates (and so it survives serialization into Supabase).
const MONTH_NAMES = ["January","February","March","April","May","June","July","August","September","October","November","December"];
export function takeProfitHorizon(interval, now = new Date()) {
  const monthsOut = interval === "1mo" ? 12 : 3;
  const horizon = interval === "1mo" ? "YEARLY" : "MONTHLY";
  const target = new Date(now.getFullYear(), now.getMonth() + monthsOut, 1);
  ...
  return {
    ...
    month,
    year,
    // e.g. "Take profit by October 2026 (~3 months)"
    label: `Take profit by ${month} ${year} (~${monthsOut} months)`,
  };
}
...
export async function runSignalEngine({ assetClass = "futures", symbol, interval, propRules = null }) {
  ...
  // Catalysts, started alongside the third agent so they cost no extra wall
  // clock. RECORDED ONLY -- nothing here touches the vote, the conviction or
  // the status. It is shipped to the Lab and tested against outcomes before
  // being allowed to influence a signal.
  //
  // Note this runs for OPTIONS too. sentimentAgent does not (see the ternary
  // above), so until now the largest slice of the corpus had no news input of
  // any kind.
  //
  // .catch(() => null) is not decoration: a news feed outage must never be
  // able to stop a signal being generated.
  const cleanSym = symbol.replace("=F", "").replace("-USD", "");
  const catalystP = Promise.all([
    symbolCatalyst(cleanSym, finnhubKey),
    macroCatalyst(finnhubKey),
  ]).then(([sym, macro]) => mergeCatalyst(sym, macro)).catch(() => null);

  const tech = technicalAgent(candles);
  const struct = structureAgent(candles);
  const third = await thirdAgentP;
  const catalyst = await catalystP;
  ...
  const s = struct.data;
  let plan = null;
  let planRejected = null;
  if (pm.direction === "LONG" && risk.approved) {
    // A swing low ABOVE the current price is not a stop -- price has already
    // broken through it. Taken literally it makes riskAmt NEGATIVE, which puts
    // t1 BELOW entry on a LONG, and signalLifecycle.js:79 tests t1 before the
    // stop against live price, so the signal grades `won` within seconds of
    // being written. One AAPL setup did exactly this 23 times and produced 23
    // of the 24 wins on record.
    //
    // Such a level is not structure, it is a level price has left behind, so
    // this falls back to the same fixed percentage used when there is no
    // structure at all.
    const swing = s.lastSwingLow;
    const usable = isNum(swing) && swing < price;
    const stop = usable ? swing : price * 0.995;
    const riskAmt = price - stop;
    plan = { entry: price, stop, t1: price + riskAmt * 1.5, t2: price + riskAmt * 3, rr: 3.0 };
    if (!usable && isNum(swing)) planRejected = "swing_low_above_price";
  } else if (pm.direction === "SHORT" && risk.approved) {
    const swing = s.lastSwingHigh;
    const usable = isNum(swing) && swing > price;
    const stop = usable ? swing : price * 1.005;
    const riskAmt = stop - price;
    plan = { entry: price, stop, t1: price - riskAmt * 1.5, t2: price - riskAmt * 3, rr: 3.0 };
    if (!usable && isNum(swing)) planRejected = "swing_high_below_price";
  }

  // Last line of defence, checked rather than assumed. Whatever produced the
  // numbers above, a LONG must have its stop below entry and its target above
  // it, and a SHORT the reverse. A plan that fails this is not a worse trade --
  // it is an arithmetic artifact that will grade itself the instant it is
  // written, so it must never reach the table at all.
  if (plan && !geometryValid(pm.direction, plan)) {
    planRejected = "inverted_geometry";
    plan = null;
  }
  if (plan && assetClass === "options") {
    plan.contractGuidance = contractGuidance(pm.direction, price, third?.data?.atmIV ?? null);
  }
  // V14: every INVEST (long-term) signal must say WHEN to take profit, as a real
  // calendar month/year rather than a vague "hold". Derived from the horizon the
  // signal was generated on — 1w candles = the Monthly horizon, 1mo = Yearly.
  if (plan && assetClass === "equity") {
    plan.takeProfitBy = takeProfitHorizon(iv);
  }
  ...
  return {
    ...
    // The same agents WITH their raw `data` (RSI, MACD histogram, put/call
    // ratio, ...). `agents` above stays exactly as it was because it is what
    // gets written to the DB column users read; this is additive and consumed
    // only by the Lab emitter. Without it, buildProvenance read `data` off the
    // already-stripped array above and captured null for every agent -- 620
    // signals recorded with the field the Lab most wanted empty.
    agentsFull: agents,
    // Volatility and liquidity from the candles this decision was made on --
    // computed here rather than later so it can never describe a different
    // window than the one the engine actually saw.
    regime: instrumentRegime(candles),
    risk,
    structure: { events: s.events, sweep: s.sweep, fvgs: s.fvgs, swingHigh: s.lastSwingHigh, swingLow: s.lastSwingLow },
    // Why a plan was refused, so the Lab can count rejections instead of
    // seeing an unexplained absence of geometry.
    planRejected,
