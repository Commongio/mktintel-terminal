...
import { ema, rsi, macd, bollinger } from "./indicators";
...
export const MODE_DEFAULT_INTERVAL = { futures: "15min", options: "1h" };
...
function contractGuidance(direction, price, atmIV) {
  ...
  const ivNote = atmIV != null ? (atmIV > 45 ? "IV elevated — consider spreads over naked longs" : "IV moderate") : "";
  return `${side} · 2–4 weeks out · ~0.30–0.40 delta${ivNote ? ` · ${ivNote}` : ""}`;
}
...
export async function runSignalEngine({ assetClass = "futures", symbol, interval, propRules = null }) {
  ...
  return {
    ...
    status: risk.approved && pm.direction !== "NEUTRAL" ? "FIRE" : pm.direction !== "NEUTRAL" ? "HOLD" : "SCAN",
