...
export const LEVEL_KINDS = {
  entry: { color: "#7FA9D8", style: "solid", label: "ENTRY" },
  tp: { color: "#5FCB96", style: "dashed", label: "TP" },
  sl: { color: "#E5697E", style: "dashed", label: "SL" },
  alert: { color: "#E0B25F", style: "dotted", label: "ALERT" },
  note: { color: "#A099E0", style: "dotted", label: "" },
};
...
export function makeTrendline({ symbol, from, to, label = "", color = "#A099E0" }) {
...
export function makeMarker({ symbol, time, text = "", shape = "circle", position = "aboveBar", color = "#7FA9D8" }) {
  const t = toUnix(time);
