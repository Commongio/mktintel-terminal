...
export const LEVEL_KINDS = {
  entry: { color: "#6F94BE", style: "solid", label: "ENTRY" },
  tp: { color: "#4FA97B", style: "dashed", label: "TP" },
  sl: { color: "#C9576B", style: "dashed", label: "SL" },
  alert: { color: "#C9A15B", style: "dotted", label: "ALERT" },
  note: { color: "#8B84C4", style: "dotted", label: "" },
};
...
export function makeTrendline({ symbol, from, to, label = "", color = "#8B84C4" }) {
...
export function makeMarker({ symbol, time, text = "", shape = "circle", position = "aboveBar", color = "#6F94BE" }) {
  const t = toUnix(time);
