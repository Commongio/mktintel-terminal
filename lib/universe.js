...
// V13.5: per-asset-class MAX signal timeframe.
//   options → 2 weeks max: high-risk/unpredictable, so no month+/year-horizon
//             holds. Allows intraday through daily/short-swing candles.
//   futures → 1 day max: funded-account day-trading instruments — signals are
//             intraday only, nothing that implies an overnight+ hold.
//   equity  → long-horizon by nature (portfolio growth): the full ladder.
// One source of truth so the Bot's interval ladder (UI) and the cron's
// portfolio sweeps (server) can never drift out of agreement.
export const ALLOWED_INTERVALS = {
  futures: ["1min", "5min", "15min", "1h", "4h"],          // intraday only (<= 1 day)
  options: ["1min", "5min", "15min", "1h", "4h", "1d"],    // up to ~2-week swing
  // V14: "1d" removed. A daily-candle hold is a swing trade, not a long-term
  // investment, and the weekly take-profit horizon it fed was explicitly dropped —
  // INVEST is Monthly (1w candles) and Yearly (1mo candles) only.
  equity:  ["1w", "1mo"],                                  // monthly / yearly holds
};
export function intervalAllowed(assetClass, interval) {
  const list = ALLOWED_INTERVALS[assetClass];
  return list ? list.includes(interval) : true;
}

// V13: major cash-settled index options — the highest-volume, most-watched
// options products that exist (SPX alone clears more premium than most single
// names combined). Given a guaranteed always-scanned slot below (like the
// day's most-actives), but deliberately NOT a conviction boost — see the
// "prioritized but not fully prioritized" note on scanUniverse().
export const PRIORITY_INDEX_OPTIONS = ["^SPX", "^NDX", "^RUT", "^VIX"];
...
const TIER_OF = (() => {
  ...
  // Indices are historically the safest, most liquid options products that
  // exist — every risk profile (including Conservative/Beginner) should see
  // them, so they're tiered "large" like the other mega-caps.
  PRIORITY_INDEX_OPTIONS.forEach((s) => (m[s] = "large"));
  return m;
})();
...
export function scanUniverse(assetClass, extras = [], cursor = 0) {
  ...
  // V13: major index options get their OWN always-on slot — separate from the
  // PINNED_MOVERS budget so a hot mover never bumps SPX out of a run. This is
  // "special attention", not an override: they still go through the exact same
  // engine/risk-gate as everything else (see writeIfChanged in the cron route),
  // just guaranteed a scan every run instead of waiting for the rotation.
  return [...new Set([...PRIORITY_INDEX_OPTIONS, ...pinned, ...slice])];
}
