...
export async function gradeSignalLifecycle(admin) {
  ...
  try {
    const { data, error } = await admin
      .from("signals").select("id,mae_r,mfe_r,peak_gain_pct,peak_loss_pct,bars_seen")
  ...
  if (excursionsAvailable) {
    for (const s of gradable) {
      ...
      // And in percent of entry, which is the only unit that makes a 10% move
      // and a 100% move distinguishable. R answers "how did this do against
      // its own risk"; percent answers "did this thing actually run", and a
      // breakout is invisible in R alone when the stop is wide.
      const forPct = s.direction === "LONG"
        ? ((px - entry) / entry) * 100
        : ((entry - px) / entry) * 100;
      ...
      excursion.set(s.id, {
        ...
        peak_gain_pct: round4(Math.max(Number(was.peak_gain_pct) || 0, forPct, 0)),
        peak_loss_pct: round4(Math.max(Number(was.peak_loss_pct) || 0, -forPct, 0)),
