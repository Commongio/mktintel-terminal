...
export const SYMBOL_MAP = { NQ: "NQ=F", MNQ: "MNQ=F", ES: "ES=F", MES: "MES=F", CL: "CL=F", GC: "GC=F", YM: "YM=F", RTY: "RTY=F", BTC: "BTC-USD", ETH: "ETH-USD" };
...
async function yahooQuotes(symbols) {
  // V12: added 52wk range + fundamentals fields (marketCap/PE/EPS) for the ticker
  // Overview's Key Stats + Fundamentals panels. Additive — other consumers ignore them.
  const url = `${YF}/v7/finance/quote?symbols=${encodeURIComponent(symbols.map(toYahoo).join(","))}&fields=regularMarketPrice,regularMarketChange,regularMarketChangePercent,regularMarketOpen,regularMarketDayHigh,regularMarketDayLow,regularMarketVolume,regularMarketPreviousClose,shortName,longName,marketState,fiftyTwoWeekHigh,fiftyTwoWeekLow,marketCap,trailingPE,epsTrailingTwelveMonths,averageDailyVolume3Month`;
