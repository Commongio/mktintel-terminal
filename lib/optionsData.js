...
import { gammaExposure } from "./gamma.js";
...
export async function fetchChainSummary(symbol) {
  ...
  // Dealer gamma, derived from this same chain. Yahoo returns no greeks, so
  // it is computed from Black-Scholes on the IV already fetched -- no extra
  // request, and no chance of describing a chain other than the one this
  // decision saw.
  const gex = gammaExposure({ calls, puts, expirationDate: options.expirationDate }, price);
