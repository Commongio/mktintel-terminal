...
export const VIDEO_THEMES = [
  // Emptied deliberately. Every export below still exists and behaves
  // correctly against an empty registry -- isVideoTheme() returns false for
  // everything, videoThemeSrc() returns null, and THEME_LIST simply contains
  // the canvas themes. Deleting the module instead would have meant touching
  // four files to remove an import that already degrades cleanly.
  //
  // Anyone whose saved theme was a video is handled without a special case:
  // migrateTheme() in page.js resolves a persisted id against THEME_LIST and
  // falls back to aurora when it does not resolve. That is exactly the
  // black-screen guard it was written for, and it now catches all eight.
  //
  // The add-a-theme instructions above still apply if videos ever come back.
];
