...
const TERMINAL_TOOLS = [
  ...
  { name: "set_theme", description: "Change the terminal background theme. All are lightweight canvas animations. none = classic dot-grid; midnight = drifting fog and ambient dust (the signature theme); obsidian = near-black volcanic glass, almost still; carbon = brushed carbon fibre with a slow reflection; ambient = additive bokeh drifting through a soft light; eclipse = off-screen light and shadow; globe = a turning wireframe world shedding stars; dots = a field of dots with a wave travelling through it; pulse = energy pulses through a still network; blackhole = a drifting gravitational lens bending a starfield; aurora = slow ribbons.", input_schema: { type: "object", properties: { themeId: { type: "string", enum: ["none", "midnight", "obsidian", "carbon", "ambient", "eclipse", "globe", "dots", "pulse", "blackhole", "aurora"] } }, required: ["themeId"] } },
