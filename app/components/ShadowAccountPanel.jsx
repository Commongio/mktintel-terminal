...
const outcomeColor = (o) =>
  o === "WIN" || o === "WINNING" ? "#5FCB96" :
  o === "STOPPED" || o === "LOSING" ? "#E5697E" : "#9DB4CC";

// ─── SHADOW ACCOUNT PANEL ─────────────────────────────────────────────────────
export default function ShadowAccountPanel({ accent = "#5CBFB1", T, assetClass = "futures" }) {
    ...
    <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 12, overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderBottom: `1px solid ${border}` }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={clear} style={{ fontFamily: FM, fontSize: 8, color: "#E5697E", background: "none", border: "1px solid rgba(255,61,87,0.25)", borderRadius: 5, padding: "3px 8px", cursor: "pointer" }}>CLEAR</button>
        </div>
      </div>
      ...
      {stats ? (
        <div style={{ display: "flex", gap: 22, padding: "12px 14px", borderBottom: `1px solid ${border}`, flexWrap: "wrap" }}>
          {[
            ...
            ["WIN RATE", `${stats.winRate}%`, stats.winRate >= 55 ? "#5FCB96" : stats.winRate >= 45 ? "#E0B25F" : "#E5697E"],
            ["AVG MOVE", `${stats.avgMove > 0 ? "+" : ""}${stats.avgMove}%`, stats.avgMove >= 0 ? "#5FCB96" : "#E5697E"],
            ...(stats.highConvWinRate != null ? [["70%+ CONV WR", `${stats.highConvWinRate}%`, stats.highConvWinRate >= 55 ? "#5FCB96" : "#E0B25F"]] : []),
          ].map(([l, v, c]) => (
            <div key={l}>
          ...
          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", borderBottom: `1px solid ${border}55` }}>
            <div>
              <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
                <span style={{ fontFamily: FM, fontSize: 10, fontWeight: 700, color: e.direction === "LONG" ? "#5FCB96" : "#E5697E" }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              {e.movePct != null && (
                <div style={{ fontFamily: FM, fontSize: 9, color: e.movePct >= 0 ? "#5FCB96" : "#E5697E" }}>
...
// ─── PAPER TRADING PANEL ──────────────────────────────────────────────────────
export function PaperTradingPanel({ accent = "#5CBFB1", T, paperMode, setPaperMode, assetClass = "futures" }) {
  const surface = T?.surface ?? "#0A1018";
  const border  = T?.border  ?? "#24313F";
  const text    = T?.text    ?? "#E2EDF8";
  const dim     = T?.dim     ?? "#9DB4CC";
    ...
    <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 12, padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", gap: 22, marginBottom: 12, flexWrap: "wrap" }}>
        {[
          ...
          ["PAPER P&L", `${totalPnl >= 0 ? "+" : ""}$${totalPnl.toFixed(0)}`, totalPnl >= 0 ? "#5FCB96" : "#E5697E"],
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
          ...
          <span style={{ fontFamily: FM, fontSize: 9, fontWeight: 700, color: liveUnlocked ? "#5FCB96" : "#E0B25F" }}>
        ...
        <div style={{ height: 5, background: "#24313F", borderRadius: 3, overflow: "hidden" }}>
          <div style={{
            ...
            background: liveUnlocked ? "#5FCB96" : "#E0B25F", borderRadius: 3, transition: "width 0.6s",
          }} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button onClick={reset} style={{
          fontFamily: FM, fontSize: 8, color: "#E5697E", background: "none",
